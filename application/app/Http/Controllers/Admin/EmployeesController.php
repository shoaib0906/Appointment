...
class EmployeesController extends Controller
{
    ...
    public function index()
    {
        ...
        else
        {
            ...
            $employees = Employee::where('executive_id','=',$executive_id->id)->get();
            //$clients = Client::where('email','=',Auth::user()->email)->get();           
        }
        
        //dd($employees[0]->reporting_boss->first_name);
    ...
    public function create()
    {
		...
		$relations = [
            'client' => \App\CLient::select(
                DB::raw("CONCAT(first_name,' ',last_name) AS name"),'id')->get()->pluck('name', 'id'),
        ];
        //dd($relations['client']);
    ...
    public function store(StoreEmployeesRequest $request)
    {
        $this->validate(request(), [
            ...
            'email' => 'required|string|email|max:255|unique:users|unique:employees',
        ...
        $employee = Employee::create($request->only(['executive_id','first_name', 'last_name', 'phone', 'email']));
		/*$services_ids = [];
    ...
    public function edit($id)
    {
        ...
        $client = \App\CLient::select(DB::raw("CONCAT(first_name,' ',last_name) AS name"),'id')->get()->pluck('name', 'id');

        return view('admin.employees.edit', compact('employee','client'));
    }
    ...
    public function show($id)
    {
        ...
        $id = $id->executive_id;
        $relations = [
            'working_hours' => \App\WorkingHour::where('executive_id', $id)->get(),
	...
	{
		$employees = DB::table('clients')->join('working_hours', function ($join) use ($request) {
			$join->on('clients.id', '=', 'working_hours.executive_id')
